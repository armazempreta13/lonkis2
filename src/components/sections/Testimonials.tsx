...
const getInitial = (name = 'Usuário') => name.trim().charAt(0).toUpperCase();

const mapToReview = (item: any, index: number, isFromGoogle = false): Review => ({
  name: item.name || 'Cliente',
  rating: Math.min(Math.max(item.rating ?? 5, 1), 5),
  text: item.text || 'Comentário indisponível',
  initial: getInitial(item.name),
  color: COLORS[index % COLORS.length],
  profilePhotoUrl: item.image || item.profilePhotoUrl || undefined,
  time: item.time || item.relativeTimeDescription || undefined,
  isFromGoogle,
});
...
export const Testimonials = () => {
  ...
  const { reviews: googleReviews, ratingData, isLoading, error } = useGoogleReviews();

  const fallbackReviews = useMemo(() => {
    return (siteConfig.pages.home.testimonials?.items || []).map((item: any, index: number) => mapToReview(item, index, false));
  }, []);

  const [reviews, setReviews] = useState<Review[]>(fallbackReviews);
  const [overallRating, setOverallRating] = useState<number>(
    Number(siteConfig.pages.home.testimonials?.googleRating ?? 5.0)
  );
  const [totalReviewsCount, setTotalReviewsCount] = useState<number>(
    Number(siteConfig.pages.home.testimonials?.totalReviews ?? 0)
  );

  useEffect(() => {
    if (googleReviews && googleReviews.length > 0) {
      setReviews(googleReviews.map((item, idx) => mapToReview(item, idx, true)));
    } else {
      setReviews(fallbackReviews);
    }
  }, [googleReviews, fallbackReviews]);

  useEffect(() => {
    if (ratingData?.rating) {
      setOverallRating(ratingData.rating);
      setTotalReviewsCount(ratingData.totalReviews ?? totalReviewsCount);
    }
  }, [ratingData, totalReviewsCount]);

  const dataSourceLabel = googleReviews && googleReviews.length > 0 ? 'Google' : 'Fallback';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-4 sm:mb-12 md:mb-16 gap-2 sm:gap-6 md:gap-8">
          ...
          <div className="flex flex-col items-stretch gap-2.5 sm:gap-4 w-full md:items-end md:w-auto">
            <div className="flex items-center justify-center sm:justify-start gap-2 sm:gap-3 bg-white/5 px-3 sm:px-5 md:px-6 py-1.5 sm:py-2 md:py-3 rounded-lg sm:rounded-xl md:rounded-2xl border border-white/10">
              <div className="flex gap-0.5 sm:gap-1">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 ${i < Math.round(overallRating) ? 'fill-yellow-400 text-yellow-400' : 'text-white/30'}`}
                  />
                ))}
              </div>
            ...
            >
              <MessageCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" /> Ver avaliações
            </Button>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="flex justify-center items-center py-12 sm:py-16 md:py-20">
            ...
            <p className="text-white/40 text-xs sm:text-sm">Buscando avaliações do Google...</p>
          </div>
        ) : (
          <>
            {error && (
              <p className="text-yellow-300 text-xs sm:text-sm mb-4">Não foi possível carregar avaliações do Google. Mostrando conteúdo local.</p>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
              {reviews.map((review, index) => (
                <motion.div
                  ...
                  className="bg-white p-4 sm:p-5 md:p-6 lg:p-8 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-[2rem] shadow-xl border border-gray-100 text-left hover:shadow-2xl transition-all duration-700 flex flex-col h-full"
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 md:gap-4 mb-4 sm:mb-5 md:mb-6">
                    {review.profilePhotoUrl ? (
                      <img
                        ...
                        alt={review.name}
                        className="w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full object-cover shadow-sm"
                        referrerPolicy="no-referrer"
                      />
                    ) : (
                      <div className={`w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-white font-bold text-sm sm:text-base md:text-lg ${review.color} shadow-sm`}>
                        {review.initial}
                      </div>
                    )}
                      ...
                      <div className="flex items-center gap-1 sm:gap-1.5">
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_, starIndex) => (
                            <Star
                              key={starIndex}
                              className={`w-2.5 h-2.5 sm:w-3 sm:h-3 ${starIndex < review.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                            />
                          ))}
                        </div>
                        {review.time && <span className="text-[9px] sm:text-xs text-gray-500">{review.time}</span>}
                      </div>
                    </div>
                    <span className="text-[9px] sm:text-xs font-bold text-blue-600">{dataSourceLabel}</span>
                  </div>

                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed line-clamp-5 sm:line-clamp-6">{review.text}</p>
                  ...
                  <div className="mt-4 pt-2 border-t border-gray-100">
                    <div className="flex items-center gap-1 sm:gap-1.5 text-[8px] sm:text-[9px] md:text-[10px] text-gray-500 font-semibold uppercase tracking-wider">
                      <CheckCircle2 className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5 text-blue-500" />
        ...
        >
          <button
            ...
            onClick={() => window.open(siteConfig.contact.mapsUrl, '_blank')}
          >
